...
<section class="py-20 px-6">
  <div class="max-w-6xl mx-auto">
              ...
              <div class="relative z-10 p-6 lg:p-8 flex flex-col h-full">
                <div class="flex items-start justify-between mb-6">
                  <div class="flex items-center gap-4">
                    <div class="relative">
                      ...
                      <div class="relative w-14 h-14 bg-linear-to-br from-highlight/20 to-highlight/5 rounded-xl flex items-center justify-center border border-highlight/30 group-hover:border-highlight/50 transition-all duration-500 shadow-inner">
                    ...
                    <div>
                      ...
                      {#if project.featured}
                        <div class="flex items-center gap-1.5 mt-2 px-2.5 py-0.5 rounded-full bg-highlight/10 border border-highlight/20 w-fit">
                          ...
                          <span class="text-[10px] font-mono text-highlight uppercase tracking-widest font-bold">Featured_Project</span>
                        </div>
                      {/if}
                    </div>
                  </div>
                ...
                <div class="flex-1">
                  ...
                  <div class="flex flex-wrap gap-2.5 mb-8">
                    {#each project.technologies as tech}
                      <div class="px-3 py-1 rounded-md bg-highlight/10 border border-highlight/20 text-xs font-mono text-highlight uppercase tracking-tighter">
                        {tech}
                      </div>
                    {/each}
                  </div>
                ...
                <div class="mt-auto pt-6 flex items-center gap-4 border-t border-border/30">
                  {#if project.githubLink}
                    <Button variant="outline" size="sm" href={project.githubLink} target="_blank" rel="noopener noreferrer" class="rounded-full border-border/50 hover:border-accent hover:bg-accent hover:text-accent-foreground transition-all duration-300">
                      <GithubIcon width="14" height="14" class="mr-2" />
                      Source_Code
                    </Button>
                  {/if}
                  {#if project.demoLink}
                    <Button size="sm" href={project.demoLink} target="_blank" rel="noopener noreferrer" class="rounded-full bg-highlight text-highlight-foreground hover:bg-highlight/90 transition-all duration-300 shadow-lg shadow-highlight/20">
    ...
    <div use:reveal={{ delay: 200 }} class="text-center mt-12">
      ...
      <Button variant="outline" href="https://github.com/KronosWasTaken" target="_blank" rel="noopener noreferrer" class="rounded-full hover:bg-accent hover:text-accent-foreground hover:border-accent transition-all">
